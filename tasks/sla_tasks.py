...
def check_sla_deadlines() -> dict:
    ...
    for fu in open_followups:
        ...
        if current_time >= deadline and not fu.sla_breached:
            ...
            stats["breached"] += 1
            logger.warning(
                f"[sla] SLA breached | followup={fu.id} session={fu.session_id}"
            )
            ...
            if fu.session:
                alert, created_alert = Alert.objects.get_or_create(
                    ...
                    defaults=dict(
                        ...
                        description=(
                            f"Follow-up #{fu.id} has exceeded its SLA deadline."
                        ),
                        send_email=True,
                    ),
                )
        ...
        # Send reminder at or after 50% if not reminded yet
...
def fix_stuck_active_sessions() -> dict:
    ...
    This handles AGI crashes, server restarts, or callers who hung up
    without a clean session_end call.
    """
...
def _send_sla_reminder(followup, pct: float) -> None:
    ...
    try:
        ...
        recipients = [
            p.notify_email or p.user.email
            for p in prefs
            if (p.notify_email or p.user.email)
        ]

        if not recipients:
            logger.info(
                f"[sla] no reminder recipients found for followup={followup.id}"
            )
            return
        ...
        if followup.session:
            alert, _ = Alert.objects.get_or_create(
                ...
                defaults=dict(
                    ...
                    title=(
                        f"SLA {level} — "
                        f"{followup.session.caller_number} ({pct:.0f}% elapsed)"
                    ),
